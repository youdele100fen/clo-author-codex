...
In Claude Code this is a hard guard enforced by `session-guard.py` through `PreToolUse`. In Codex, treat the same state file as a session instruction unless the active plugin runtime confirms `PreToolUse` hook enforcement. Generated projects still include the upstream hook for round-trip compatibility with Claude Code.
